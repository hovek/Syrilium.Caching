...
namespace Syrilium.Common
{
    public class Cryptography : ICryptography
    {
        public string GetMD5Hash(string text)
        {
            ...
            return convertHashBytesToString(retVal);
        }

        public string GetMurmur3Hash(string text)
        {
            var murmur3 = new Murmur3();
            byte[] retVal = murmur3.ComputeHash(Encoding.UTF8.GetBytes(text));
            return convertHashBytesToString(retVal);
        }

        private string convertHashBytesToString(byte[] hash)
        {
